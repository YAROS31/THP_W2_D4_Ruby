...
def nb_handle
  puts "1. Il y a #{$journalists.count} handles dans cette array"
end
nb_handle

#2. Quel est le handle le plus court de cette liste ?#
def handle_court
  sorted_by_length = $journalists.sort_by{|i| i.length}
  puts "2. #{sorted_by_length[0]} correspond au handle le plus court"

  #--bonus, faire apparaître tous les éléments qui ont min+1 caractère--# 
  tableau_court = [] #je crée mon tableau des min.length
  tableau_court << sorted_by_length[1]
  j = 1
  while 
    sorted_by_length[j].length == sorted_by_length[j+1].length
    tableau_court << sorted_by_length[j+1]
    j += 1
  end
  print "2. Bonus : Voici un tableau de tous les handles de min+1 caractères : #{tableau_court}\n"
  #--fin du bonus--# 
end
handle_court
...
def nb_handles_5car
  aaa = $journalists.count{|i| i.length == (5+1)}
  puts "3. Il y a #{aaa} handles comprenant 5 caractères ('@' exclu)."
end
nb_handles_5car

#4. Combien commencent par une majuscule (première lettre juste après le @) ?
def nb_handle_maj
  aab = $journalists.count{|i| i[1] =~ /[A-Z]/}
  puts "4. Il y a #{aab} handles qui commencent par une majuscule."
end
nb_handle_maj

#5. Trie la liste de handle par ordre alphabétique.
def handle_alpha
  aac = $journalists.sort_by {|i| i.downcase}
  puts "5. voici le tableau #{aac}"
  puts " "
end
handle_alpha
...
def handle_sorted
  puts "6. voici le tableau trié par taille : #{$journalists.sort_by{|i| i.length}}"
end
handle_sorted

#7. Quelle est la position dans l'array de la personne @epenser ?
def find_epenser
  puts "7. @epenser est en position #{$journalists.index("@epenser")} dans l'array"
end
find_epenser
